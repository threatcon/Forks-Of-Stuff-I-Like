...
export function activate(context: vscode.ExtensionContext) {
  ...
  //   --------------book
  context.subscriptions.push(
    vscode.commands.registerCommand('document.export', handler.handlerBookExportModules)
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('document.import', handler.handlerBookImportModules)
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('document.compile', handler.handlerBookCompile)
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('document.exportFrx', handler.handlerBookExportFrxModules)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'DiffView.refreshModification',
      handler.handlerDiffViewRefreshModification
    )
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('DiffView.collapseAll', handler.handlerDiffViewCollapseAll)
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('DiffView.pushSrc', handler.handlerDiffViewCommitModule)
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('DiffView.pullSrc', handler.handlerDiffViewPullModule)
  );
  ...
  // define at tree
  context.subscriptions.push(
    vscode.commands.registerCommand('DiffView.diffBaseTo', (resource) =>
  ...
  VbeOutput.showInfo('vba-manager extension start', false);
}
...
const displayMenus = (isOn: boolean) => {
  vscode.commands.executeCommand('setContext', 'manager.showVbsCommand', isOn);
