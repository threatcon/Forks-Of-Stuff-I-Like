...
export const FOLDER_SUFFIX_SRC = '.src';
...
export function getVbecmDirs(bookPath: string, suffix: string = STRING_EMPTY) {
  ...
  const srcDir = path.resolve(fileDir, baseName + FOLDER_SUFFIX_SRC);
...
export async function resolveVbeConflicting(
  ...
  if (isBase && isVbe) {
    ...
    const r = await modalDialogShow('Do you want to copy the module in the vbe to the base.', autoDialog);
    ...
    const r = await modalDialogShow(
      'Do you want to delete the modules in the base and source.',
      autoDialog
    );
    ...
    // copy frx ,too
    const r = await modalDialogShow(
      'Do you want to copy the module in the vbe to the base and the source.',
      autoDialog
    );
    r && copyModuleSync(moduleFileName, vbeDir, baseDir);
